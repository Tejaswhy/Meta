...
def grade_obstacle_avoidance(observation: dict, action: Action) -> float:
    ...
    left_lane_clear = bool(
        observation.get("left_lane_clear", False)
    )
    right_lane_clear = bool(
        observation.get("right_lane_clear", False)
    )
    ...
    if front_distance <= 3.0:
        ...
        if action.action_type == "change_lane":
            ...
            action.action_type == "brake"
            and action.brake >= 0.5
        ):
            score = 0.8
        else:
            score = 0.1
...
def grade_signal_handling(observation: dict, action: Action) -> float:
    ...
    crossing = bool(
        observation.get("pedestrian_crossing", False)
    )
